...
class TFTPServer(private val send: (TFTPPacket) -> Unit) {
        ...
        while(true) {
            ...
            while (!shutdownTransfer && (dataPacket == null || dataPacket.address != twrp.address || dataPacket.port != twrp.port)) {
                if (dataPacket != null) {
                    ...
                    send(TFTPErrorPacket(dataPacket.address, dataPacket.port, TFTPErrorPacket.UNKNOWN_TID, "Unexpected Host or Port"))
                }
                dataPacket = try {
