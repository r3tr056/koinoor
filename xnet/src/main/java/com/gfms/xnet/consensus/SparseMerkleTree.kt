...
class SparseMerkleTree(input: Map<Int, ByteArray>, val depth: Int = 256, val hash: (ByteArray) -> ByteArray = { data -> SHA256Digest.digest(data) }) {
    ...
    fun proofForIndex(idx: Int): List<ByteArray> {
        ...
        for (level in 0 until (depth - 1)) {
            ...
            index /= 2
            proof += if (tree[level].containsKey(siblingIndex)) {
