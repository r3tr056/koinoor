...
/**
 * NOTICE : This is a fake repo for search development.
 */
object SearchRepo {
    ...
    suspend fun search(query: String): List<CryptoCoin> = withContext(Dispatchers.Default) {
        delay(200L) // simulate an I/O delay
        cryptoLs.filter { it.name.contains(query, ignoreCase = true)}
    }
}
