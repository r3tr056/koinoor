package com.gfms.koinoor.ui.theme
...
private val LightColorPalette = KoinoorColors(
    brand = Shadow5,
...
private val DarkColorPalette = KoinoorColors(
    brand = Shadow1,
...
fun KoinoorTheme(
    ...
    ProvideKoinoorColor(colors = colors) {
        MaterialTheme(
...
object KoinoorTheme {
    val colors: KoinoorColors
        @Composable
        get() = LocalKoinoorColors.current
}
...
class KoinoorColors(
    ...
    fun update(other: KoinoorColors) {
    ...
    fun copy(): KoinoorColors = KoinoorColors(
...
@Composable
fun ProvideKoinoorColor(
    colors: KoinoorColors,
    content: @Composable () -> Unit
) {
    val colorPalette = remember { colors.copy() }
    colorPalette.update(colors)
    CompositionLocalProvider(LocalKoinoorColors provides colorPalette, content = content)
}

private val LocalKoinoorColors = staticCompositionLocalOf<KoinoorColors> {
    error("No KoinoorColorPalette provided")
}
